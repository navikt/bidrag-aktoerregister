...
public class AktoerRegisterControllerMock {
  ...
  @Autowired
  public AktoerRegisterControllerMock(AktoerRepository aktoerRepository, HendelseRepository hendelseRepository) {
    TSSService tssService = new TSSServiceMock();
    TPSService tpsService = new TPSServiceMock();
    this.aktoerregisterService = new AktoerregisterServiceImpl(aktoerRepository, hendelseRepository, tpsService, tssService);
  }
  ...
  public ResponseEntity<List<HendelseDTO>> hentHendelser(@PathVariable(name = "fra") int fra, @PathVariable(name = "antall") int antall) {
