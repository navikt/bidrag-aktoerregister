...
enum class Gradering(val diskresjonskode: Diskresjonskode, val omtaltSom: String) {
    ...
    companion object {
        fun valueOf(type: String?): Gradering? = values().find { it.name == type }

        fun from(diskresjonskode: Diskresjonskode?): Gradering? = values().find { it.diskresjonskode == diskresjonskode }
    }
}
